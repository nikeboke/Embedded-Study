...
void print_array (unsigned char *array, unsigned int roller){
...
unsigned char find_median (unsigned char *array, unsigned int roller){
  unsigned char median = 0;
  median = *(array + (roller / 2) -1);
  return median;
}

float find_mean (unsigned char *array, unsigned int roller){
  ...
  printf("acc = %d \n", accumulator);
  mean = accumulator / ((float) roller); //must type cast one of the two integers to float for accurate calculation.
  return mean;
}

unsigned char find_maximum (unsigned char *array, unsigned int roller){
  unsigned char maximum = *array; // initalize the maximum variable with the value of the first array element
  for (int i=1; i<roller; i++){ // start the loop from the second elemnt
    if (*(array + i) > maximum){
      maximum = *(array + i);
    }
    else {} // do nothing.
...
unsigned char find_minimum (unsigned char *array, unsigned int roller){
...
void sort_array (unsigned char *array, unsigned int roller){
  ...
  do {
    ...
    for (int index=0; index<roller; index++){
